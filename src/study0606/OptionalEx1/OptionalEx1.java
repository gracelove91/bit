...
public class OptionalEx1 {
	public static void main(String[] args) {
		...
		int result1 = Optional.of("123").filter(x -> x.length() > 0).map(Integer::parseInt).orElse(-1);
		...
		OptionalInt optInt2 = OptionalInt.empty();	//빈객체생성.
		
		System.out.println(optInt1.isPresent());
		System.out.println(optInt2.isPresent());
		
		System.out.println(optInt1.getAsInt());
//		System.out.println(optInt2.getAsInt());		//OptionalInt.empty(); -> NoSuchElementException
		...
		Optional<String> opt2 = Optional.empty();			//빈객체생성.
